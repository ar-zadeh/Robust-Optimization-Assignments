...
Variables
 ...
 epsilonplus(s)  "its hard to explain! But its positive :)))"
 ;
...
objective..      z =e= lambda * sum(s, senarioObj(s)* pi(s))+gama * sum(s, pi(s)*(sqr(senarioObj(s)-sum(sPrime, pi(sPrime)*senarioObj(sPrime)))))+omega* sum(s,pi(s)*sqr(epsilonplus(s)))
...
*cons1(s)..       e(s) =e= sum(j, g(j) * y(j)) + sum((i,j), c(i,j) * x(i,j,s)) + sum((j,k),  a(j,k) * u(j,k,s));



option MIQCP=BARON;
Model stochastic /all/ ;
stochastic.reslim=200;
stochastic.optCR= 0;
Solve stochastic using MIQCP minimizing z ;

Display z.l, y.l, x.l, u.l ,epsilonplus.l;
